...
function LoginPage() {
  ...
  return <div className='md:w-1/2 lg:w-2/6 bg-gray-400 mx-auto'>
    ...
    {/* sign in with google */}
    <div className='flex rounded-md items-center bg-blue-500'>
      {/* icon */}
      <FcGoogle className='bg-white'/>
      {/* text */}
      <span className='text-lg font-semibold'>{t('LoginPage.Google.label')}</span>
    </div>
    <Formik initialValues={{ email: '', password: '' }}
      ...
      <Form className='font-medium text-lg flex flex-col gap-5 w-full items-center'>
        <div className='w-full'>
          <label htmlFor="email" className='inline-block'>{t('LoginPage.email.label')}</label>
